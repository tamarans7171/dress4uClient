...
function Filters(props) {
  ...
  const [filterStyles, setFilterStyles] = useState([])
    ...
    console.log(temp);
    let tempStyles = checkedStyles.map((b, i) => {
      if (b)
        return styles[i]._id
  ...
  return (
    ...
    <h3>סינון לפי סגנון</h3>
        {styles&&styles.map((s,i)=><div key={i} className="checkbox-wrapper-39">
        
          <label>
      <input 
      onChange={(e)=>{updateStyles(i, e.target.checked) }} 
      value={s._id}  type="checkbox"/>
  ...
  <hr/>
  <h3>סינון לפי צבע</h3>
  <CirclePicker color={colorChoosen} onChangeComplete={ handleChangeComplete } colors={colors}/>
