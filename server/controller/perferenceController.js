...
const checkUserPerferenceByDress = async (req, res) => {
    ...
    try {
        ...
        let perference = await PerferenceModel.find({user:user, dress:dress})
            console.log(res);
            res.json({ans:perference.length > 0 , perference:perference.length > 0? perference[0] : null})
...
module.exports = {checkUserPerferenceByDress, getPerferencees, getPerferenceById, deletePerference, addPerference, updatePerference }
